    ...
    extends BasicJitterBufferBehaviour
{
    ...
    /**
     * The number of packets during the receipt of which statistics are to be gathered for the
     * purposes of shrinking.  Note that whilst the jitter buffer contains more than half its
     * maximum capacity, it will shrink at double this rate.
     */
    ...
    public AudioJitterBufferBehaviour(RTPSourceStream stream)
    {
        ...
        AJB_SHRINK_DECREMENT
            = com.sun.media.util.Registry.getInt(
                    ...
                    1);
        // As per comment for AJB_SHRINK_INTERVAL above, when the jitter buffer is over half its
        // maximum capacity we shrink at one in every 15 packets (double speed).
        AJB_SHRINK_INTERVAL
            = com.sun.media.util.Registry.getInt(
    ...
    protected int monitorQSize(Buffer buffer)
    {
        ...
        if (AJB_ENABLED)
        {
             ...
             * Shrink at double speed if we're over half the max capacity.
             */
            int shrinkThreshold = (size > AJB_MAX_SIZE / 2) ? (AJB_SHRINK_INTERVAL / 2) : AJB_SHRINK_INTERVAL;

            if ((shrinkCount >= shrinkThreshold) &&
                (size > AJB_MIN_SIZE) &&
                q.freeNotEmpty())
            {
    ...
    private void shrink(int capacity)
    {
        ...
        Log.info("Shrinking packet queue to " + capacity + ", fill count is " + q.getFillCount());
