...
public class PlaybackEngine extends BasicController implements ModuleListener
{
    ...
    class HeavyPanel extends java.awt.Panel implements VisualContainer
    {
        ...
        public HeavyPanel(Vector<Component> visuals)
        {
        }
    }
    ...
    class LightPanel extends java.awt.Container implements VisualContainer
    {
        ...
        public LightPanel(Vector<Component> visuals)
        {
        }
    }
    ...
    class PlayerTControl extends BasicTrackControl implements Owned
    {
        ...
        /**
         * Returns true if this track holds the primary time base.
         */
        @Override
        public boolean isTimeBase()
        {
            ...
            {
                if (this.modules.elementAt(j) == primarySink)
                    return true;
            }
            return false;
        }
    ...
    class ProxyClock implements Clock
    {
        Clock primary, current;
        BasicClock backup;
        ...
        protected void reset(boolean usePrimary)
        {
            if (primary != null && usePrimary)
                current = primary;
            else
            {
                ...
                if (primary != null)
                {
                    synchronized (backup)
                    {
                        ...
                        backup.setMediaTime(primary.getMediaTime());
                        if (started)
        ...
        public void setPrimary(Clock primary)
        {
            this.primary = primary;
            current = (primary == null ? backup : primary);
            if (primary != null)
            {
                try
                {
                    backup.setTimeBase(primary.getTimeBase());
    ...
    protected Vector<StateTransistor> modules;
    protected Vector<BasicFilterModule> filters;
    protected Vector<BasicSinkModule> sinks;
    protected Vector<BasicSinkModule> waitPrefetched;
    protected Vector<BasicSinkModule> waitStopped;
    protected Vector<BasicSinkModule> waitEnded;

    protected Vector<BasicSinkModule> waitResetted;
    protected Track tracks[];
    protected Demultiplexer parser;
    protected BasicSinkModule primarySink = null;
    protected BasicSourceModule source;
    protected ProxyClock proxyClock;
    ...
    static boolean USE_PRIMARY = true;
    ...
    public PlaybackEngine(BasicPlayer p)
    {
        ...
        setClock(proxyClock = new ProxyClock());
    ...
    protected GraphNode buildTrackFromGraph(BasicTrackControl tc, GraphNode node)
    {
        ...
        while (true)
        {
            ...
            if (dst instanceof BasicFilterModule)
                filters.addElement((BasicFilterModule) dst);
            else if (dst instanceof BasicSinkModule)
                sinks.addElement((BasicSinkModule) dst);
    ...
    protected Component createVisualContainer(Vector<Component> visuals)
    {
    ...
    protected boolean doConfigure1()
    {
        ...
        modules = new Vector<StateTransistor>();
        filters = new Vector<BasicFilterModule>();
        sinks = new Vector<BasicSinkModule>();
        waitPrefetched = new Vector<BasicSinkModule>();
        waitStopped = new Vector<BasicSinkModule>();
        waitEnded = new Vector<BasicSinkModule>();
        waitResetted = new Vector<BasicSinkModule>();
    ...
    protected boolean doConfigure2()
    {
        ...
        {
            Track primary = FramePositioningAdapter.getPrimaryTrack(tracks);
            if (primary != null)
            {
    ...
    {
        // Locate the set the primary time base.
        if (!manageTimeBases())
        {
            Log.error(realizeError);
            Log.error(timeBaseError);
    ...
    public synchronized float doSetRate(float r)
    {
        ...
        // primary clock and let it determine the actual rate use.
        if (primarySink == null)
            r = getClock().setRate(r);
        else
            r = primarySink.doSetRate(r);
     ...
     */
    protected BasicSinkModule findPrimarySink()
    {
        // Search for all the enabled track for a primary time base.
    ...
    /**
     * Search and update the primary time base.
     */
    boolean manageTimeBases()
    {
        // Obtain a primary time base from one of its SinkModules.
        primarySink = findPrimarySink();

        return updatePrimaryTimeBase();
    }
        ...
        if (src instanceof BasicSinkModule)
        {
            synchronized (waitEnded)
            {
                ...
                if (waitEnded.isEmpty())
                {
                    ...
                    proxyClock.reset(USE_PRIMARY);
                } else if (src == primarySink)
                {
                    // If the primary sink has finished but the
                    // not all the other tracks are finished, we'll
                    // need to switch the clock to using a backup for
                    // the rest of the media.
                    proxyClock.reset(USE_BACKUP);
                }
            }
        }
    }
    ...
    /**
     * Update the primary timebase on all the modules.
     */
    boolean updatePrimaryTimeBase()
    {
        BasicSinkModule bsm;
        int size = sinks.size();

        if (primarySink != null)
            proxyClock.setPrimary(primarySink.getClock());
        else
            // No primary sink found in the module, we'll use the
            // system time base.
            proxyClock.setPrimary(null);

        // Set the primary time base to each of the SinkModules.
        for (int i = 0; i < size; i++)
        {
            bsm = (BasicSinkModule) sinks.elementAt(i);
            if (bsm != primarySink && !bsm.prefetchFailed())
            {
                try
                {
                    bsm.setTimeBase(proxyClock.getTimeBase());
                } catch (IncompatibleTimeBaseException e)
                {
                    return false;
                }
