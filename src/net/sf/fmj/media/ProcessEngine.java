...
public class ProcessEngine extends PlaybackEngine
{
    class ProcGraphBuilder extends SimpleGraphBuilder
    {
        ...
        GraphNode buildCustomGraph(Format in)
        {
            ...
            if (codecs != null)
            {
                ...
                for (int i = 0; i < codecs.length; i++)
                {
                    ...
                    node.level = 0;
                    candidates = new Vector<GraphNode>();
                    candidates.addElement(node);
                }
            }
         ...
         * candidates.
         */
        void doGetSupportedOutputFormats(Vector<GraphNode> candidates, Vector<Format> collected)
        {
            ...
            for (int i = 0; i < outs.length; i++)
            {
                ...
                Vector<String> cnames = PlugInManager.getPlugInList(outs[i], null,
                        PlugInManager.CODEC);
    ...
    class ProcTControl extends BasicTrackControl implements Owned
    {
        ...
        private Format checkSize(Format fmt)
        {
            ...
            if (fmt.matches(new VideoFormat(VideoFormat.H263_RTP))
        ...
        public boolean isTimeBase()
        {
            ...
            {
                if (modules.elementAt(j) == primarySink)
                    return true;
            }
            return false;
        }
        ...
        {
            Log.comment("Setting codec chain, engine state is: " +
                                engine.getState());
            if (engine.getState() > Configured)
            {
                Log.error("Error setting codec chain for: " +
                                  Arrays.toString(codec));
                throwError(new NotConfiguredInCodecChainError(connectErr));
            }
            if (codec.length < 1)
        ...
        {
            Log.comment("Setting renderer, engine state is: " +
                                engine.getState());
            if (engine.getState() > Configured)
        ...
        Format[] verifyMuxInputs(ContentDescriptor cd, Format inputs[])
        {
            ...
            Vector<String> cnames = PlugInManager.getPlugInList(null, cd,
    ...
    protected Vector<String> targetMuxNames = null;
            ...
            throws NotConfiguredError
    {
        if (getState() < Configured)
            throwError(new NotConfiguredError("getSupportedContentDescriptors "
                    + NOT_CONFIGURED_ERROR));
        Vector<String> names = PlugInManager.getPlugInList(null, null,
                PlugInManager.MULTIPLEXER);
            ...
            throws NotConfiguredError
    {
        ...
        if (ocd != null)
        {
            Vector<String> cnames = PlugInManager.getPlugInList(null, ocd,
                    PlugInManager.MULTIPLEXER);
