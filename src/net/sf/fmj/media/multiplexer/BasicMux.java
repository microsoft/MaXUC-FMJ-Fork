        ...
        javax.media.Multiplexer, Clock
{
    protected class BasicMuxDataSource extends PushDataSource
    {
        ...
        public BasicMuxDataSource(ContentDescriptor cd)
        {
            this.cd = cd;
        }
    ...
    protected class BasicMuxPushStream implements PushSourceStream
    {
        ...
        public void setTransferHandler(SourceTransferHandler sth)
        {
            synchronized (writeLock)
            {
                ...
                if (requireTwoPass)
                {
                    if (sth instanceof RandomAccess)
                    {
    ...
    protected final Integer sourceLock = new Integer(0);
    ...
    private boolean compensateStart(Buffer buffer, int trackID)
    {
        synchronized (dataLock)
        {
                ...
                if (!firstBuffersDone[trackID])
                {
                    ...
                    } else
                    {
                        if (buffer.getFormat() instanceof VideoFormat)
                        {
                            ...
                            boolean isKey = (rgbFmt.matches(fmt) || yuvFmt
                                    .matches(fmt));
                            if (isKey
                        ...
                        // compensated.
                        for (boolean b : firstBuffersDone)
                        {
                            if (!b) return true;
                        }
            ...
            if (!done)
            {
                ...
                return (!mClosed) && (firstBuffers[trackID] != null);
            }
            ...
            for (Buffer value : firstBuffers)
            {
                if (value.getFormat() instanceof AudioFormat)
                {
                    masterTime = value.getTimeStamp();
                    break;
                }
                if (value.getTimeStamp() < masterTime)
                    masterTime = value.getTimeStamp();
            }
    ...
    public DataSource getDataOutput()
    {
        if (source == null)
        {
            source = new BasicMuxDataSource(outputCD);
            synchronized (sourceLock)
            {
                sourceLock.notifyAll();
            }
