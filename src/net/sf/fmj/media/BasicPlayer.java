        ...
        ControllerListener, DownloadProgressListener
{
    ...
    protected Vector<Controller> controllerList = new Vector<>();
    private Vector<Controller> optionalControllerList = new Vector<>();
    private Vector<Controller> removedControllerList = new Vector<>();
    private Vector<Controller> currentControllerList = new Vector<>();
    private Vector<String> potentialEventsList = null;
    private Vector<Controller> receivedEventList = new Vector<>();
    private boolean receivedAllEvents = false;
    private Vector<String> configureEventList = new Vector<>();
    private Vector<String> realizeEventList = new Vector<>();
    private Vector<String> prefetchEventList = new Vector<>();
    private Vector<String> stopEventList = new Vector<>();
    private Controller restartFrom = null;
    private Vector<Controller> eomEventsReceivedFrom = new Vector<>();
    private Vector<Controller> stopAtTimeReceivedFrom = new Vector<>();
     ...
     * Player will:
     * <ul>
     * <li>Fix the Controller to the Player's time-base.
            ...
            throws IncompatibleTimeBaseException
    {
        ...
        Log.comment("Player engine init state is: " + playerState);
        ...
        Log.comment("Controller engine init state is: " + controllerState);
        ...
        {
            // The secondary does not support the primary's rate.
    ...
    protected void completeRealize()
    {
        ...
        try
        {
            secondaryToPrimaryTimeBase(getPrimaryTimeBase());
     ...
     * overridden by subclass. The subclass method should change the primary
    ...
    protected/* synchronized */boolean doPrefetch()
    {
        ...
        Vector<Controller> list = controllerList;
     ...
     */
    public final Vector<Controller> getControllerList()
    {
    ...
    {
        int state = setAndLogEngineChange(getState());
        if (state < Realized)
        {
            throwError(new NotRealizedError(
    ...
    /**
     * Return the list of controls from its secondary controllers plus the ones that
     ...
     * return the primary TimeBase -- the TimeBase that all other controllers
     * are fixed to. Use SystemTimeBase if unsure.
     *
     * @return the primary time base.
     */
    protected abstract TimeBase getPrimaryTimeBase();
     ...
     */
    private Vector<Controller> getReceivedEventsList()
    {
    ...
    public Time getStartLatency()
    {
        ...
        // Find the longest start latency from all the secondary Controllers.
    ...
    private/* synchronized */void notifyIfAllEventsArrived(
            Vector<Controller> controllerList, Vector<Controller> receivedEventList)
    {
    ...
    protected/* synchronized */void processEvent(ControllerEvent evt)
    {
        ...
        Vector<String> eventList = potentialEventsList;
     ...
     * the timebase can be none other than the primary timebase as returned by
     * getPrimaryTimeBase(). This is to ensure that we won't set to a timebase
     ...
     * @exception IncompatibleTimeBaseException
     *                thrown when a time base other than the primary time base is
     *                set when audio is enabled.
     */
    @Override
    public void setTimeBase(TimeBase tb) throws IncompatibleTimeBaseException
    {
        TimeBase oldTimeBase = getPrimaryTimeBase();
    ...
    /**
     * Fix all the controllers to the primary time base. The controllers should
     ...
     *                timebase.
     */
    protected void secondaryToPrimaryTimeBase(TimeBase tb)
            throws javax.media.IncompatibleTimeBaseException
    {
        // $$ System.out.println("secondaryToPrimaryTimeBase: primary timebase is " +
        // tb);
        // $ System.out.println("Setting primary " + tb + " on " + this);
    ...
    {
        int state;
        state = setAndLogEngineChange(getState());
