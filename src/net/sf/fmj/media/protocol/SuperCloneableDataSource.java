 ...
 * on the CloneableDataSource (primary) will propagate the same calls to the
 * cloned (replica) DataSources.
 *
...
{
    class PushBufferDataSourceReplica extends PushBufferDataSource
    {
        PushBufferStream[] streams = null;

        public PushBufferDataSourceReplica()
        {
        ...
        public void connect() throws IOException
        {
            for (int i = 0; i < streams.length; i++)
            {
                ((SourceStreamRepeater) streams[i]).connect();
            }
        }
        ...
        public void disconnect()
        {
            for (int i = 0; i < streams.length; i++)
            {
                ((SourceStreamRepeater) streams[i]).disconnect();
            }
        }
    ...
    class PushDataSourceReplica extends PushDataSource
    {
        PushSourceStream[] streams = null;

        public PushDataSourceReplica()
        {
     ...
     */
    private Vector<DataSource> clones = new Vector<DataSource>();
     ...
     * pulled.
     *
     * @return a clone DataSource for this DataSource.
     */
    javax.media.protocol.DataSource createClone()
    {
        DataSource newReplica;

        if ((input instanceof PullDataSource)
                || (input instanceof PushDataSource))
            newReplica = new PushDataSourceReplica();
        else
            // input is a Buffer type DataSource
            newReplica = new PushBufferDataSourceReplica();

        clones.addElement(newReplica);
