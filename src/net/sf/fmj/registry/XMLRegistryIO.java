...
class XMLRegistryIO implements RegistryIO
{
    ...
    private Element getMimeElement(Document document)
    {
        ...
        final Iterator<String> typesIterator = registryContents.mimeTable
                .getMimeTypes().iterator();
        while (typesIterator.hasNext())
        {
            final String type = (String) typesIterator.next();
            final List<String> extensions = registryContents.mimeTable
                    .getExtensions(type);
            ...
            ClassNotFoundException
    {
        ...
        final List<Element> list = getChildren(element, ELEMENT_DEVICE);
        for (int i = 0; i < list.size(); i++)
        {
            ...
            final List<Element> formatElementsList = getChildren(deviceElement,
    ...
    private void loadMimeTypes(Element element)
    {
        ...
        final List<Element> list = getChildren(element, ELEMENT_MIMETYPE);
        for (int i = 0; i < list.size(); i++)
        {
            ...
            final List<Element> list2 = getChildren(typeElement, "ext");
    ...
    public void write(OutputStream os) throws IOException
    {
        ...
        try
        {
            serializer.setOutputProperty(
                    "{https://xml.apache.org/xalan}indent-amount", "4");
        } catch (Exception e)
        {
        }
